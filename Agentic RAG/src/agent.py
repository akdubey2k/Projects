...
class AgenticAI:
    ...
    def __init__(self, rag_system):
        # object, allows the agent to query a knowledge base for relevant information.
        # Stores the RAG system as an instance variable, making it accessible to other methods in 
        # the class
        self.rag_system = rag_system
        # agent is not created and defers agent creation to the create_agent method with a language 
        # model.
        self.agent = None
    ...
    def create_agent(self, llm):
        # creates a list of Tool objects, with a single tool that wraps the RAG system’s query method.
        tools = [
        ...
        # fetches a pre-defined ReAct prompt template, which instructs the language model to reason
        # step-by-step, decide whether to use tools, and format responses (e.g., using [THOUGHT], 
        # [ACTION], [OBSERVATION] tags).
